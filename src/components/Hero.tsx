...
const Hero: React.FC = () => {
  ...
  const slogan = config?.basic?.hero?.slogan || 'The most popular '
  const description = config?.basic?.hero?.description || 'The largest Football API covering 1000+ leagues & cups. Livescore, pre-match odds, events, line-ups, players, statistics, transfers, predictions and more...'
  const appName = config?.basic?.app_name || 'Football API'
  return (
    <section className="relative bg-cover bg-center bg-no-repeat py-32 md:py-48" style={{ backgroundImage: "url('https://picsum.photos/seed/stadium/1600/900')" }}>
      ...
      <div className="relative container mx-auto px-4 text-center">
        <h1 className="text-4xl md:text-6xl font-black text-white leading-tight">
          {slogan} <span className="text-api-green">{appName}</span>
        </h1>
